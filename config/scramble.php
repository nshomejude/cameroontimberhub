...
/*
 * Tool choice (Task 0.3 / blueprint §4 "API-First, API-as-a-Product"):
 *
 * dedoc/scramble was evaluated against darkaonline/l5-swagger and chosen.
 * Every existing app/Http/Controllers/Api/V1/* action already returns a
 * typed API Resource (or a typed FormRequest ->validated() shape) and every
 * request is validated by a dedicated FormRequest with real Laravel
...
return [
    ...
    // Documentation is public and read-only by design (blueprint §4: "no
    // auth needed to browse the spec") — it describes the API, it is not
    // the API. RestrictedDocsAccess (env/gate-restricted) is intentionally
    // NOT used here so `GET /docs/api` works the same in every environment.
    'middleware' => [
        'web',
    ],
