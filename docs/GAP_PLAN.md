...
| 0.9b | **Commercial/billing engine.** The rest of `docs/PRICING_SPEC.md`: a multi-segment plan/entitlement data model (buyer/dealer/exporter/verification/compliance/traceability/data/education plans, not just supplier), an entitlement engine consumed identically by web/PWA/API (§22), marketplace commission calculation with caps (§15), a tax engine (§20), invoicing/refunds/credit-notes with immutable historical invoices (§21), pricing governance/versioning/audit (§23), and real payment-provider integration. This is a genuinely large, separate commercial-infrastructure build — the pricing page (0.9) can be accurate without it, and building fake checkout/billing against no real payment provider decision would be exactly the kind of fabricated infrastructure this project's rules prohibit. | Real subscriptions/checkout/entitlements for every segment beyond the existing supplier plans depend on this. | TBD — needs a payment-provider decision first |
