...
| 1.5.2 | ✅ **DONE** — new `/buy-cameroon-wood` domestic search (`DomesticMarketplaceController` + `DomesticMarketplaceService`, plain GET query-string filtering, no Livewire), entirely read-only against the already-live `Product`/`Company`/`ProductType` — species, grade, dimensions (max thickness), quantity (against listing MOQ), treatment (moisture content), region and delivery-time filters/facets, sorted by newest/price/name/fastest-delivery. `product_type` stands in for the 1.5.1 Category tree until that item wires in (tracked as 1.5.2b below); no export vocabulary (FOB/Incoterms/Export) appears anywhere on the page. `ProductController`/`ProductCatalogueService`/`Product`/`Company`/`ProductType` left untouched. Plan: `docs/superpowers/plans/2026-08-29-domestic-search.md`. Commits `136df34`, `5b71c56`. | 8 |
| 1.5.2b | Wire `/buy-cameroon-wood` domestic search to the 1.5.1 Category tree once fully adopted, replacing the interim `product_type` facet with true category-tree filtering. | 2 |
...
| 1.5.5 | ✅ **DONE** — investigation found the `rfqs.type` prerequisite from 1.5 was never actually built (no column, no enum existed) and, separately, that the RFQ wizard's multi-line-item support ("500 doors + 200 windows") already existed (`rfq_items` hasMany, repeater UI, `RfqWizard::MAX_ITEMS`) — so this item needed only an additive `rfqs.type` column/enum, not a new line-item model. Added `RfqType` enum (Export/DomesticManufacturing), additive `rfqs.type` migration (default `export`, CHECK constraint), `Rfq::scopeOfType()`, `RfqWizard::type()`/`putType()`, and a new `/request-quote/manufacturing` entry point (`RfqController::createManufacturing()`) that reuses the existing export wizard end-to-end, tagging the submission's `type`. "Local Procurement Hub" and "Project RFQ" are treated as the same mechanism (a multi-line RFQ) under this one new type — no separate entry point was justified by the schema. Plan: `docs/superpowers/plans/2026-08-29-manufacturing-rfq.md`. Commits `155ffe5` (wizard/controller/route/test); `RfqType`/migration/model-cast/scope/`RfqTypeTest` landed identically via a concurrent agent's `511afed`. | 8 |
...
| 1.5.7 | ✅ **DONE** — "Made in Cameroon" is a **computed** eligibility fact, not a manually-reviewed `BadgeService`/`VerificationBadge` document badge (that system is built entirely around approved-document requirements and doesn't fit a rule like "verified domestic manufacturer"). Added `Product::qualifiesForMadeInCameroon()` + `Product::scopeMadeInCameroon()` (company `country_code = CM`, `status = Verified`, `type` in {Manufacturer, Processor, Artisan}, product `status = Active`) and a new `/made-in-cameroon` landing page (`MadeInCameroonController`) listing qualifying products — this is the filter. Proven against dev data: 4 qualifying products, all from a verified manufacturer. `BadgeService`/`BadgeType`/`Company`/`OrganisationType` left untouched. Plan: `docs/superpowers/plans/2026-08-29-made-in-cameroon-badge.md`. Commits `6db63c1` (plan), `7df011b` (Product.php method + route landed via a concurrent 1.5.1 commit), `8421560` (controller/view/tests). **Follow-up (not done):** product QR showing transformation history is deferred — it depends on shipment/checkpoint tracking (1.5.10/1.5.11), which does not exist yet. | 5 |
| 1.5.8 | ✅ **DONE** — investigated `CompanyGallery`/`company_gallery` (per-image, sort-ordered, plan-capped, company-scoped) and found it already matches a "portfolio item" shape (`caption` doubles as title); extended it additively rather than building a parallel `Portfolio` model. Added nullable `description`, `materials_used`, `completed_on` and a boolean `is_portfolio` flag to `company_gallery` (existing rows/consumers unaffected — flag defaults false); `CompanyGallery::scopePortfolio()`; one additive `Company::portfolioItems()` relation. New public page `GET /companies/{slug}/portfolio` (`CompanyController::portfolio()`), gated to `OrganisationType::Artisan` companies (404 for any other type, matching `show()`'s non-disclosure pattern). **Dev-data note:** no `Artisan`-typed company exists yet in dev data, so the live page could not be demoed end-to-end against real seed data — proven instead via feature tests seeding an artisan company + portfolio/non-portfolio gallery rows. Plan: `docs/superpowers/plans/2026-08-29-artisan-profiles.md`. Commits `efa7740` (plan), `ffed424` (migration/model), `130d801` (`Company::portfolioItems`), `5401471` (route/controller/view/tests). | 6 |
| 1.5.9 | ✅ **DONE** — Logistics directory filtering `Company` by `OrganisationType::Logistics`, with "Trusted"/"Tech-enabled" verification tiers computed over the Phase 0 `Verification` framework. Read-only, mirrors the Transformation Network directory's house pattern. Commit `3acf585`. | 0 (done) |
| 1.5.10 | ✅ **DONE** — `RfqType::Transport` case, a `/request-quote/transport` entry point mirroring the manufacturing wizard, `Shipment` model with a digital waybill (QR linking cargo product IDs, reusing the `endroid/qr-code` package from item 0.8), and `ShipmentService::createFromOrder()`. Vehicle/driver linkage is nullable/optional — a Shipment is fully functional with zero fleet data (mirrors `Inventory`'s opt-in pattern), so it doesn't block on item 1.5.12. Status/location tracking is deliberately NOT built here — that's item 1.5.11's job (a generic polymorphic primitive, not hard-wired to Shipment). Commits `8f3f30a`→`389e753`. | 0 (done) |
| 1.5.11 | ✅ **DONE** — built as a generic polymorphic primitive (`checkpoint_updates.trackable_type`/`trackable_id`, mirroring `Consent`'s `subject_type`/`subject_id`) rather than hard-wired to `Shipment`, since `Shipment` (1.5.10) was being built concurrently and might not exist yet. `CheckpointUpdate` model + `HasCheckpointUpdates` trait, `App\Enums\TrackingCheckpointStatus` (dispatched/in_transit/delayed/delivered — named distinctly from the pre-existing, unrelated `App\Enums\CheckpointStatus` used by `VerificationCheckpoint`, after a collision was caught and reverted mid-implementation), nullable `latitude`/`longitude`/`photo_path` for the human-attached GPS+photo at update time. A shared `tracking_token` identifies a trackable's whole checkpoint history (deliberately not row-unique — every checkpoint for one trackable shares it — fixed after a duplicate-key test failure caught the wrong constraint). `CheckpointTracker` service + public `GET /track/{token}` route/controller/view mirror `ReceiptVerifier`'s token-lookup and allow-list-payload discipline exactly, throttled via a new `checkpoint-track` rate limiter. Proven end-to-end in `tests/Feature/CheckpointUpdateTest.php` using `Company` as a test-fixture trackable — no `Shipment` file was touched. Plan: `docs/superpowers/plans/2026-08-29-checkpoint-tracking.md`. Commit `cc4a3ad`. **Follow-up (not done):** now that `Shipment` (1.5.10) exists, add `use HasCheckpointUpdates;` to `App\Models\Shipment` to make it a real trackable — that one-line wiring was intentionally left out of this item's scope. | 0 (done) |
| 1.5.12 | ✅ **DONE** — `Vehicle`/`Driver` models (registration, license, `company_id`) both using the existing `HasDocuments` trait for compliance paperwork, so expiry alerts come for free from the already-built `SendDocumentExpiryReminderJob` (item 0.1b) rather than a second parallel system. Company-facing registry list view at `GET /fleet`. Commits `f4652b3`, `96d5a02`. | 0 (done) |
