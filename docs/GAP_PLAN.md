...
| 0.2b | **BLOCKED — plan's badge-issuance hypothesis was wrong, needs redesign before dispatch.** A dispatched agent investigated before writing any code (correctly) and found the real system is materially more complex than `docs/superpowers/plans/2026-08-28-verification-consumer-migration.md` assumed: `companies.status` is a real 6-value machine (`draft, pending, verified, suspended, rejected, archived`, including a `suspended→verified` reactivation path, not just draft/pending/verified/rejected). Worse, `CompanyVerified`/`BadgeIssued` do NOT fire from a single company-status transition as the plan assumed — the real trigger is `ApproveVerificationRequest` → `VerificationService::approve()` → a **per-badge-type document-requirements evaluation loop** inside `BadgeService::issue()`, plus a second, separate dispatch site in `app/Actions/Company/VerifyCompany.php` the plan never accounted for at all. Re-wiring this onto `VerificationFlowService`'s linear 8-stage sequence is not a like-for-like swap; it needs a genuinely redesigned plan, not a patched one. | §9's single verification framework only genuinely covers "supplier" once `Company` is on it too — but forcing this migration on the wrong hypothesis risks silently changing when real companies get verification badges. | 6 (plan needs a rewrite before re-estimating) |
...
| 0.4b-ii | **Contact-form consent — blocked on a schema decision.** `ContactController::store()` still validates and discards its own `consent` checkbox. Unlike the RFQ/inquiry flows, the contact form has no persisted entity to attach a `Consent` row to (`consents.subject_type`/`subject_id` are `NOT NULL`) — it only sends an email. Needs a human decision: make `consents.subject_id` nullable (affects the existing partial unique/active indexes), or persist a `ContactMessage` model purely to hang consent off it. Neither picked here. | Same GDPR-style consent gap as the RFQ/inquiry forms, on the one remaining flow. | 1–2 depending on which option is chosen |
...
| 0.8c | **Certificate allocation carry-over across versions, and KMS/HSM signing upgrade.** Two known limitations from 0.8's Digital Core, both stated plainly in the code rather than left implicit. (1) `certificate_allocations` rows reference a specific certificate *version* row and are not automatically carried forward when `CertificateService::createVersion()` supersedes it (see `database/migrations/2026_08_29_100001_create_certificate_allocations_table.php`'s docblock) — this needs a deliberate product decision on whether a re-issued version inherits its predecessor's claimed quantity or starts clean. (2) `CertificateSigningService` signs with an application-level Ed25519 key held in a file on the application server: genuinely separated from the database (an admin browsing Filament cannot read it) but not from the application server itself. A real KMS/HSM integration — where the application asks a signing service to sign and never holds the private key — is the documented next step. | Neither is a fabricated gap; both are consequences of 0.8 shipping a real digital core without pretending to hardware-backed key custody or a settled versioning policy. | TBD — depends on a KMS provider decision and an allocation carry-over policy |
