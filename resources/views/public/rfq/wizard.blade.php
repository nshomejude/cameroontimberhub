...
<x-layouts.app
    ...
    <section class="border-b border-sand-200 dark:border-[#2c2a24] bg-gradient-to-b from-forest-50 to-sand-50 dark:from-forest-950 dark:to-[#14130f]">
        <div class="mx-auto max-w-6xl px-4 py-8 sm:py-10">
            ...
            <p class="mt-2 max-w-2xl text-[0.9375rem] text-ink-soft dark:text-[#b3ab9b]">
    ...
    <div class="mx-auto max-w-6xl px-4 py-6 sm:py-8">
        ...
        @if (session('rfq_notice'))
            <p role="status" class="mt-4 rounded-xl border border-timber-200 bg-timber-50 px-4 py-3 text-[0.8125rem] text-timber-900">
                {{ session('rfq_notice') }}
            </p>
        @endif
        ...
        @if ($errors->any())
            <div role="alert" tabindex="-1" id="rfq-error-summary"
                ...
                <p class="text-[0.875rem] font-semibold text-red-800 dark:text-red-300">
                ...
                <ul class="mt-1.5 list-disc space-y-0.5 pl-5 text-[0.8125rem] text-red-800 dark:text-red-300">
        ...
        <div class="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start">
            <main class="min-w-0 rounded-2xl border border-sand-200 dark:border-[#2c2a24] bg-white dark:bg-[#1f1d18] px-5 py-6 sm:px-7 sm:py-7">
                <p class="text-[0.75rem] font-semibold uppercase tracking-wide text-forest-700 dark:text-forest-400">
                ...
                <p class="mt-1 text-[0.875rem] text-ink-soft dark:text-[#b3ab9b]">{{ $stepCaption }}</p>
                    ...
                    <div class="flex flex-wrap items-center gap-3 border-t border-sand-200 pt-5 dark:border-[#2c2a24]">
                        @if ($previous)
                            <button type="submit" name="direction" value="back"
                                    class="inline-flex items-center gap-2 rounded-full border border-sand-300 px-5 py-2.5 text-[0.875rem] font-semibold text-ink transition hover:bg-sand-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-forest-500 focus-visible:ring-offset-2 dark:border-[#3a352e] dark:text-[#e4ddcf] dark:hover:bg-[#26241e]">
                                <x-heroicon-m-arrow-left class="h-4 w-4" /> Back
                            </button>
                        @endif

                        <button type="submit" name="direction" value="next"
                                class="inline-flex items-center gap-2 rounded-full bg-forest-700 px-6 py-2.5 text-[0.875rem] font-semibold text-white transition hover:bg-forest-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-forest-500 focus-visible:ring-offset-2">
                            @if ($step === 'review')
