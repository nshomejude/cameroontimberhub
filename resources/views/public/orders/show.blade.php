...
<x-layouts.app
    ...
    <div class="mx-auto max-w-6xl px-4 py-8 sm:py-12">
        ...
        @if (session('quote_notice'))
            <div role="status" class="mb-4 rounded-2xl border border-forest-200 bg-forest-50 px-4 py-3 text-[0.875rem] text-forest-900 dark:border-forest-900 dark:bg-[#1b2c22] dark:text-forest-200">
        ...
        <section class="rounded-2xl border border-sand-200 bg-white p-5 dark:border-[#2c2a24] dark:bg-[#1f1d18] sm:p-7">
            <div class="flex flex-wrap items-start gap-4">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="mt-1 max-w-2xl text-[0.9375rem] text-ink-soft dark:text-[#8f887b]">
                ...
                <div class="flex flex-wrap gap-2 print:hidden">
                    ...
                    <a href="{{ $access->link(request(), 'responses', $rfq) }}"
                       class="inline-flex items-center gap-2 rounded-full border border-sand-300 px-5 py-2.5 text-[0.875rem] font-semibold text-ink transition hover:border-forest-400 dark:border-[#3a372f] dark:text-[#e4ddcf]">
                        All responses
                    </a>
                </div>
            </div>
        ...
        <div class="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
                ...
                <section aria-labelledby="order-supplier"
                    ...
                    <div class="mt-4 flex flex-wrap items-start justify-between gap-4">
                        <div class="min-w-0">
                            ...
                            <div class="mt-2 space-y-1 text-[0.875rem] text-ink-soft dark:text-[#8f887b]">
                        ...
                        @if ($company?->slug)
                            <a href="{{ route('companies.show', $company->slug) }}"
                               class="inline-flex items-center gap-2 rounded-full border border-sand-300 px-4 py-2 text-[0.8125rem] font-semibold text-ink transition hover:border-forest-400 dark:border-[#3a372f] dark:text-[#e4ddcf]">
                                ...
                                @foreach ($order->items as $i => $item)
                                    <tr class="align-top text-ink dark:text-[#e4ddcf]">
                                        ...
                                        <td class="py-3 pr-3 font-medium">
                                            {{ $item->description }}
                                            @if ($item->species_name)<span class="block text-[0.8125rem] font-normal text-ink-soft dark:text-[#8f887b]">{{ $item->species_name }}</span>@endif
                                        </td>
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <section aria-labelledby="order-progress"
                            ...
                            @foreach ($milestones as $milestone)
                                <li class="flex items-start gap-3 text-[0.875rem]">
                                    ...
                                    <span class="flex-1">
                                        ...
                                        <span class="block text-[0.8125rem] text-ink-soft dark:text-[#8f887b]">
                        ...
                        @if ($order->status === \App\Enums\OrderStatus::Cancelled && $order->cancellation_reason)
                            <p class="mt-4 rounded-xl bg-red-50 px-4 py-3 text-[0.875rem] text-red-800 dark:bg-red-950 dark:text-red-200">
            ...
            <aside class="space-y-4">
                ...
                <section class="rounded-2xl border border-sand-200 bg-white p-5 dark:border-[#2c2a24] dark:bg-[#1f1d18]">
                    ...
                    <p class="mt-4 text-[0.75rem] leading-relaxed text-ink-soft dark:text-[#b3ab9b]">
                ...
                @if ($receipt)
                    <section class="rounded-2xl border border-sand-200 bg-white p-5 dark:border-[#2c2a24] dark:bg-[#1f1d18]">
                        ...
                        <p class="mt-3 text-[0.875rem] text-ink-soft dark:text-[#8f887b]">Receipt number</p>
                        <p class="font-mono text-[0.9375rem] font-semibold text-forest-800 dark:text-forest-300">{{ $receipt->receipt_number }}</p>
                        <p class="mt-2 text-[0.8125rem] text-ink-soft dark:text-[#8f887b]">
                            Issued {{ $receipt->issued_at->isoFormat('D MMM YYYY, HH:mm') }}
                        </p>
                        <a href="{{ $access->link(request(), 'receipt', $rfq) }}"
                           class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full border border-sand-300 px-4 py-2.5 text-[0.875rem] font-semibold text-ink transition hover:border-forest-400 dark:border-[#3a372f] dark:text-[#e4ddcf]">
                            Open receipt
                        </a>
                    </section>
                @endif
                ...
                <section class="rounded-2xl border border-sand-200 bg-white p-5 dark:border-[#2c2a24] dark:bg-[#1f1d18]">
                    ...
                    <ol class="mt-3 list-decimal space-y-2 pl-4 text-[0.875rem] text-ink-soft dark:text-[#8f887b]">
