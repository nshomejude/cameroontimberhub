        ...
        <article class="receipt-sheet mt-4 rounded-2xl border border-sand-200 bg-white p-5 text-ink dark:border-[#2c2a24] dark:bg-[#1f1d18] dark:text-[#e4ddcf] sm:p-8">
            ...
            <header class="grid gap-6 border-b border-sand-200 pb-6 dark:border-[#2c2a24] md:grid-cols-[1.4fr_1fr]">
                ...
                <div class="md:text-right">
                    ...
                    @if ($receipt)
                        <p class="mt-3 text-[0.875rem] uppercase tracking-wide text-ink-soft dark:text-[#8f887b]">Receipt no.</p>
                        <p class="font-mono text-lg font-bold text-forest-800 dark:text-forest-300">{{ $receipt->receipt_number }}</p>
                        <p class="mt-2 inline-block rounded-lg bg-forest-800 px-3 py-1.5 text-[1.0625rem] font-medium text-white">
            ...
            <section class="grid gap-6 border-b border-sand-200 py-6 dark:border-[#2c2a24] md:grid-cols-3">
                <div>
                    <h2 class="text-[0.875rem] font-semibold uppercase tracking-wider text-forest-800 dark:text-forest-300">Bill to</h2>
                    <div class="mt-2 space-y-0.5 text-[1.0625rem] leading-relaxed">
                        <p class="text-[1.125rem] font-semibold">{{ $order->buyer_company ?: $order->buyer_name }}</p>
                ...
                <div>
                    <h2 class="text-[0.875rem] font-semibold uppercase tracking-wider text-forest-800 dark:text-forest-300">Supplier</h2>
                    <div class="mt-2 space-y-0.5 text-[1.0625rem] leading-relaxed">
                        <p class="text-[1.125rem] font-semibold">{{ $order->supplier_name }}</p>
                ...
                <div>
                    <h2 class="text-[0.875rem] font-semibold uppercase tracking-wider text-forest-800 dark:text-forest-300">Order information</h2>
                    <dl class="mt-2 space-y-1 text-[1.0625rem]">
            ...
            {{-- ---------------- Items ---------------- --}}
            <section class="py-6">
                <h2 class="text-[0.875rem] font-semibold uppercase tracking-wider text-forest-800 dark:text-forest-300">Order items</h2>

                <div class="mt-3 overflow-x-auto">
                    <table class="w-full min-w-[38rem] text-left text-[1.0625rem]">
                        <thead>
                            ...
                            @foreach ($order->items as $i => $item)
                                <tr class="align-top">
                                    ...
                                    <td class="px-3 py-3 font-medium">
                                        {{ $item->description }}
                                        @if ($item->species_name)<span class="block text-[0.9375rem] font-normal text-ink-soft dark:text-[#8f887b]">{{ $item->species_name }}</span>@endif
                                    </td>
            ...
            <section class="grid gap-6 border-t border-sand-200 py-6 dark:border-[#2c2a24] sm:grid-cols-2 lg:grid-cols-3">
                <div>
                    <h2 class="text-[0.875rem] font-semibold uppercase tracking-wider text-forest-800 dark:text-forest-300">Payment summary</h2>
                    <dl class="mt-2 space-y-1 text-[1.0625rem]">
                        @foreach ([
                    ...
                    <p @class([
                        'mt-3 inline-block rounded-md px-3 py-1.5 text-[0.9375rem] font-semibold',
                ...
                <div>
                    <h2 class="text-[0.875rem] font-semibold uppercase tracking-wider text-forest-800 dark:text-forest-300">Delivery summary</h2>
                    <dl class="mt-2 space-y-1 text-[1.0625rem]">
                        @foreach ([
                ...
                <div>
                    <h2 class="text-[0.875rem] font-semibold uppercase tracking-wider text-forest-800 dark:text-forest-300">Terms &amp; conditions</h2>
                    <ul class="mt-2 space-y-1.5 text-[1.0625rem] text-ink-soft dark:text-[#8f887b]">
            ...
            @if ($receipt)
                <section class="rounded-xl border border-forest-200 bg-forest-50 p-5 dark:border-forest-900 dark:bg-[#1b2c22]">
                    <h2 class="flex items-center gap-2 text-[1.0625rem] font-semibold uppercase tracking-wider text-forest-800 dark:text-forest-300">
                        <x-heroicon-m-shield-check class="h-4 w-4" /> Verify this receipt
                    </h2>
                    <div class="mt-3 grid gap-4 sm:grid-cols-2">
                        <dl class="space-y-1 text-[1.0625rem]">
                            @foreach ([
                        ...
                        <div class="text-[1.0625rem] text-ink-soft dark:text-[#8f887b]">
                            <p>Open the verification link below, or go to {{ route('receipts.verify') }} and enter the receipt number.</p>
                            <p class="mt-2 break-all font-mono text-[0.9375rem] text-forest-800 dark:text-forest-300">{{ $receipt->verificationUrl() }}</p>
                        </div>
                    </div>
                </section>
            @endif

            <footer class="mt-6 border-t border-sand-200 pt-4 text-center text-[0.9375rem] text-ink-soft dark:border-[#2c2a24] dark:text-[#8f887b]">
