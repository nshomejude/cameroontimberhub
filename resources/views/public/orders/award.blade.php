...
<x-layouts.app
    ...
    <div class="mx-auto max-w-5xl px-4 py-8 sm:py-12">
        ...
        <header class="mt-4">
            ...
            <p class="mt-2 max-w-2xl text-[1.125rem] text-ink-soft dark:text-[#8f887b]">
        ...
        <section aria-labelledby="award-effects"
            ...
            <ul class="mt-4 space-y-2.5 text-[1.125rem] text-ink dark:text-[#e4ddcf]">
                @foreach ([
            ...
            <p class="mt-4 rounded-xl bg-sand-100 px-4 py-3 text-[1.0625rem] text-ink-soft dark:bg-[#26241e] dark:text-[#b3ab9b]">
        ...
        <div class="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem]">
            ...
            <div class="space-y-6">
                ...
                <section class="rounded-2xl border border-sand-200 bg-white p-5 dark:border-[#2c2a24] dark:bg-[#1f1d18] sm:p-7">
                    ...
                    <p class="mt-1 text-[1.0625rem] text-ink-soft dark:text-[#8f887b]">Quote {{ $quote->reference_code }}</p>
                </section>
                                ...
                                @foreach ($quote->items as $i => $item)
                                    <tr class="align-top text-ink dark:text-[#e4ddcf]">
                                        ...
                                        <td class="py-3 pr-3 font-medium">
                                            {{ $item->description }}
                                            @if ($item->species) <span class="block text-[1.0625rem] font-normal text-ink-soft dark:text-[#8f887b]">{{ $item->species->common_name }}</span> @endif
                                        </td>
                ...
                <section class="rounded-2xl border border-forest-200 bg-forest-50 p-5 dark:border-forest-900 dark:bg-[#1b2c22]">
                    @if ($actionable)
                        <form method="POST" action="{{ $access->link(request(), 'accept', $rfq, $quote) }}" class="space-y-4">
                            ...
                            <label class="flex items-start gap-2.5 text-[1.0625rem] text-ink dark:text-[#e4ddcf]">
                            ...
                            <button type="submit"
                                    class="w-full rounded-full bg-forest-700 px-5 py-3 text-[1.125rem] font-semibold text-white transition hover:bg-forest-800">
                                Award order
                            </button>
                            <p class="text-[0.9375rem] text-ink-soft dark:text-[#b3ab9b]">This cannot be undone from here.</p>
                        </form>
                    @else
                        <p class="text-[1.0625rem] text-ink dark:text-[#e4ddcf]">
