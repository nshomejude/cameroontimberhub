...
<x-layouts.app
    ...
    <div class="bg-white">
        <div class="mx-auto max-w-[1400px] px-4 py-6 lg:px-6">
            ...
            {{-- ---------------- Body + rail ---------------- --}}
            {{-- The left rail is gated at xl, not lg: at 1024px three columns
                 leave the prose under 340px, which is unreadable. Below 1280px
                 the page keeps its original two-column layout untouched. --}}
            <div class="mt-8 grid gap-10 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start xl:grid-cols-[15rem_minmax(0,1fr)_20rem]">

                {{-- ---------------- Left rail: Knowledge Centre navigation ----------------
                     Desktop only, mirroring the right rail's `hidden` idiom.
