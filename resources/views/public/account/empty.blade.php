...
<x-layouts.account
    ...
    <section class="overflow-hidden rounded-2xl bg-[#032719] px-6 py-10 text-center lg:px-12 lg:py-14">
        ...
        <p class="mx-auto mt-3 max-w-xl text-[0.9375rem] leading-relaxed text-sand-300">
        ...
        <div class="mt-6 flex flex-wrap justify-center gap-3">
            <a href="{{ route('rfq.create') }}"
               class="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-[0.9375rem] font-bold text-forest-800 transition hover:bg-sand-200">
                Post an RFQ <x-heroicon-m-arrow-right class="h-4 w-4" />
            </a>
            <a href="{{ url('/marketplace') }}"
               class="inline-flex items-center gap-2 rounded-full border border-white/25 px-6 py-3 text-[0.9375rem] font-semibold text-white transition hover:bg-white/10">
    ...
    <section aria-label="How buying works" class="mt-4 grid gap-3 lg:mt-6 lg:grid-cols-3 lg:gap-6">
        @foreach ([
            ...
            <div class="rounded-2xl border border-sand-200 bg-white p-5">
                ...
                <p class="mt-4 text-[0.6875rem] font-bold uppercase tracking-[0.14em] text-timber-700">{{ $card['step'] }}</p>
                <h3 class="mt-1 font-display text-[1.0625rem] font-bold text-forest-950">{{ $card['title'] }}</h3>
                <p class="mt-2 text-[0.875rem] leading-relaxed text-ink-soft">{{ $card['body'] }}</p>
            </div>
        @endforeach
    </section>
    ...
    <p class="mt-4 rounded-2xl border border-sand-200 bg-white px-5 py-4 text-[0.8125rem] leading-relaxed text-ink-soft">
