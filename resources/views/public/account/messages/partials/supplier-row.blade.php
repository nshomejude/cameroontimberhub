...
<li class="flex items-center gap-3 px-4 py-3">
    ...
    <div class="min-w-0 flex-1">
        <p class="flex items-center gap-1.5 truncate text-[0.9375rem] font-semibold text-forest-950">
            {{ $company->name }}
    ...
    <form method="POST" action="{{ route('account.messages.start') }}" class="shrink-0">
        ...
        <button type="submit"
                class="flex items-center gap-1.5 rounded-xl bg-forest-50 px-3.5 py-2 text-[0.8125rem] font-bold text-forest-800 transition hover:bg-forest-100">
