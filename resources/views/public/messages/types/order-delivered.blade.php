...
@if ($order)
    <div class="py-1" id="m{{ $message->getKey() }}">
        <div class="mb-2 flex items-center gap-3">
            ...
            <span class="text-[0.6875rem] font-bold uppercase tracking-[0.14em] text-ink-soft">Delivery</span>
            <span class="h-px flex-1 bg-sand-300"></span>
        </div>
        ...
        <div class="rounded-2xl border border-forest-200 bg-forest-50/60 p-4 shadow-sm">
            ...
            @if ($facts !== [])
                <dl class="mt-3 grid grid-cols-2 gap-x-4 gap-y-2.5 border-t border-forest-200 pt-3 text-[0.8125rem]">
            ...
            @if ($proof->isNotEmpty())
                <div class="mt-3 border-t border-forest-200 pt-3">
                    <p class="text-[0.6875rem] font-bold uppercase tracking-wide text-ink-soft">Proof of delivery</p>
                    <ul class="mt-2 space-y-1.5">
                        @foreach ($proof as $document)
                            <li>
                                <a href="{{ route('order-documents.download', $document) }}"
                                   class="flex items-center gap-2.5 rounded-xl border border-forest-200 bg-white px-3 py-2 text-[0.8125rem] transition hover:bg-sand-50">
                                    ...
                                    <span class="min-w-0 flex-1">
                                        ...
                                        <span class="block text-[0.6875rem] text-ink-soft">{{ $document->extension() }} · {{ $document->humanSize() }}</span>
                                    </span>
        ...
        @if ($isBuyer && $order->status === \App\Enums\OrderStatus::Delivered)
            <div class="mt-2">
                <button type="button" wire:click="completeOrder({{ $order->getKey() }})"
                        class="w-full rounded-xl bg-forest-700 px-3.5 py-2.5 text-[0.875rem] font-semibold text-white">
