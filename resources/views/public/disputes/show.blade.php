<x-layouts.app title="Dispute #{{ $dispute->id }}" noindex>
    <div class="mx-auto max-w-[900px] px-4 py-8 lg:px-6">
        <a href="{{ route('disputes.index', ['order' => $order->id]) }}" class="text-sm text-forest-700 hover:underline">&larr; Back to disputes</a>

        <div class="mt-3 flex items-center justify-between">
            <h1 class="font-display text-2xl font-bold text-forest-950">Dispute #{{ $dispute->id }} — {{ $dispute->category->label() }}</h1>
            <span class="rounded-full bg-{{ $dispute->status->color() }}-100 px-3 py-1 text-xs font-semibold text-{{ $dispute->status->color() }}-800">
                {{ $dispute->status->label() }}
            </span>
        </div>

        <p class="mt-2 text-sm text-ink-soft">Raised by {{ $dispute->raisedByUser?->name ?? 'a party' }} on {{ $dispute->created_at->format('d M Y') }}</p>
        ...
        <section class="mt-6 rounded-2xl border border-sand-200 bg-white p-5">
            <h2 class="text-sm font-semibold uppercase tracking-wide text-ink-soft">Description</h2>
        ...
        @if ($dispute->status === \App\Enums\DisputeStatus::Resolved || $dispute->status === \App\Enums\DisputeStatus::Closed)
            <section class="mt-6 rounded-2xl border border-forest-200 bg-forest-50 p-5">
                <h2 class="text-sm font-semibold uppercase tracking-wide text-forest-800">Admin decision</h2>
        ...
        <section class="mt-6">
            <h2 class="text-lg font-semibold text-forest-950">Evidence</h2>
            <ul class="mt-2 space-y-2">
                @forelse ($dispute->evidence as $item)
                    <li class="rounded-lg border border-sand-200 bg-white p-3 text-sm">
                        <span class="font-semibold text-ink">{{ $item->submittedByUser?->name ?? 'A party' }}</span>
                        <span class="text-ink-soft"> — {{ $item->created_at->format('d M Y') }}</span>
                        <p class="mt-1 text-ink">{{ $item->description }}</p>
                        @if ($item->hasFile())
                            <p class="mt-1 text-ink-soft">Attached: {{ $item->original_filename }}</p>
                        @endif
                    </li>
                @empty
                    <li class="text-sm text-ink-soft">No evidence submitted yet.</li>
                @endforelse
            </ul>
            ...
            <form method="POST" action="{{ route('disputes.evidence', ['order' => $order->id, 'dispute' => $dispute->id]) }}"
                ...
                @csrf
                <textarea name="description" rows="3" placeholder="Describe this evidence" class="w-full rounded-lg border-sand-300"></textarea>
        ...
        <section class="mt-6">
            <h2 class="text-lg font-semibold text-forest-950">Messages</h2>
            <ul class="mt-2 space-y-2">
                @forelse ($dispute->messages as $message)
                    <li class="rounded-lg border border-sand-200 bg-white p-3 text-sm">
                        <span class="font-semibold text-ink">{{ $message->user?->name ?? 'A party' }}</span>
                        <span class="text-ink-soft"> — {{ $message->created_at->format('d M Y H:i') }}</span>
                        <p class="mt-1 text-ink">{{ $message->body }}</p>
                    </li>
                @empty
                    <li class="text-sm text-ink-soft">No messages yet.</li>
                @endforelse
            </ul>
            ...
            <form method="POST" action="{{ route('disputes.reply', ['order' => $order->id, 'dispute' => $dispute->id]) }}"
                ...
                @csrf
                <textarea name="body" rows="3" placeholder="Write a response" class="w-full rounded-lg border-sand-300"></textarea>
                <button type="submit" class="rounded-lg bg-forest-700 px-4 py-2 font-semibold text-white hover:bg-forest-800">
                    Send response
                </button>
            </form>
