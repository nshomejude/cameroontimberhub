@php
    ...
    $classification = array_filter([
        __('messages.species.commercial_category') => $species->commercial_category?->label(),
        __('messages.species.botanical_family') => $species->family,
        __('messages.species.density_air_dry') => $species->densityRange(),
        __('messages.species.durability_class') => $species->durability_class,
        __('messages.species.janka_hardness') => $species->janka_hardness ? number_format($species->janka_hardness).' N' : null,
        __('messages.species.trade_names') => is_array($species->trade_names) && $species->trade_names ? implode(', ', $species->trade_names) : null,
        __('messages.species.local_names') => is_array($species->local_names) && $species->local_names ? implode(', ', $species->local_names) : null,
        __('messages.species.regions_harvested') => is_array($species->region_availability) && $species->region_availability ? implode(', ', $species->region_availability) : null,
    ]);
        ...
        <div class="mx-auto max-w-[1400px] px-4 py-6 lg:px-6">
            ...
            <nav aria-label="Breadcrumb">
                <ol class="flex flex-wrap items-center gap-2 text-[0.9375rem] text-ink-soft">
                    ...
                    <li><a href="{{ route('species.index') }}" class="rounded transition hover:text-forest-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-forest-300">{{ __('messages.common.timber_species') }}</a></li>
                    <li aria-hidden="true">/</li>
            ...
            <div class="mt-4 grid gap-6 lg:grid-cols-[22rem_minmax(0,1fr)]">
                <div>
                    @if ($swatch['image'])
                        <img src="{{ $swatch['image'] }}" alt="{{ __('messages.species.grain_alt', ['name' => $species->common_name]) }}"
                    ...
                    @else
                        ...
                        <p class="mt-2 text-[0.875rem] text-ink-soft">{{ __('messages.species.illustrative_swatch') }}</p>
                    @endif
                </div>
                ...
                <div class="min-w-0">
                    ...
                    <ul class="mt-3 flex flex-wrap gap-2">
                        ...
                        @if ($species->isPremium())
                            <li class="rounded-md bg-forest-800 px-2.5 py-1 text-[0.9375rem] font-semibold text-white">{{ __('messages.common.premium') }}</li>
                        @endif
                        @if ($species->is_promoted)
                            <li class="rounded-md bg-sand-200 px-2.5 py-1 text-[0.9375rem] font-semibold text-ink">{{ __('messages.species.promoted_species') }}</li>
                        @endif
                        @if ($species->is_cites_listed)
                            <li class="rounded-md bg-timber-100 px-2.5 py-1 text-[0.9375rem] font-semibold text-timber-800">
                                {{ __('messages.species.cites_listed') }}{{ $species->cites_appendix ? ' — '.__('messages.species.cites_appendix', ['appendix' => $species->cites_appendix]) : '' }}
                            </li>
                        @endif
                    </ul>
                    ...
                    <div class="mt-6 flex flex-wrap gap-3">
                        <a href="{{ route('rfq.create', ['species' => $species->slug]) }}"
                           class="rounded-lg bg-forest-700 px-5 py-2.5 text-[1.0625rem] font-semibold text-white transition hover:bg-forest-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-forest-500 focus-visible:ring-offset-2">
                            {{ __('messages.common.request_a_quote') }}
                        </a>
                        <a href="{{ route('species.index') }}"
                           class="rounded-lg border border-sand-300 px-5 py-2.5 text-[1.0625rem] font-semibold text-ink transition hover:border-forest-600 hover:text-forest-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-forest-500">
                            {{ __('messages.common.back_to_directory') }}
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_20rem]">
                <div class="space-y-10">
                    ...
                    @if ($classification)
                        <section aria-labelledby="classification-heading">
                            <h2 id="classification-heading" class="{{ $sectionTitle }}">{{ __('messages.species.classification') }}</h2>
                            ...
                            @if ($species->commercial_category)
                                <p class="mt-3 text-[0.9375rem] text-ink-soft">
                                    {{ __('messages.species.commercial_category_note') }}
                                </p>
                            @endif
                        </section>
                    @endif
                    ...
                    <section aria-labelledby="eudr-heading">
                        <h2 id="eudr-heading" class="{{ $sectionTitle }}">{{ __('messages.species.eudr_heading') }}</h2>
                        <div class="mt-4 {{ $panel }}">
                            @if ($species->eudr_risk_note)
                                <p class="whitespace-pre-line text-[1.0625rem] text-ink">{{ $species->eudr_risk_note }}</p>
                            @else
                                <p class="text-[1.0625rem] text-ink-soft">{{ __('messages.species.eudr_not_assessed', ['name' => $species->common_name]) }}</p>
                            @endif
                        </div>
                    </section>
                    ...
                    @if (is_array($species->characteristics) && count($species->characteristics))
                        <section aria-labelledby="properties-heading">
                            <h2 id="properties-heading" class="{{ $sectionTitle }}">{{ __('messages.species.properties_heading') }}</h2>
                            <dl class="mt-4 grid gap-3 sm:grid-cols-2">
                    ...
                    @if (is_array($species->typical_uses) && count($species->typical_uses))
                        <section aria-labelledby="uses-heading">
                            <h2 id="uses-heading" class="{{ $sectionTitle }}">{{ __('messages.species.typical_uses') }}</h2>
                    ...
                    @if ($species->is_cites_listed)
                        <section aria-labelledby="cites-heading">
                            <h2 id="cites-heading" class="{{ $sectionTitle }}">{{ __('messages.species.cites_status') }}</h2>
                            <div class="mt-4 rounded-xl border border-timber-200 bg-timber-50 px-4 py-3 text-[1.0625rem] text-ink">
                                <p>{{ __('messages.species.cites_listing_line', ['name' => $species->scientific_name ?: $species->common_name, 'appendix' => $species->cites_appendix ? ' '.__('messages.species.cites_appendix', ['appendix' => $species->cites_appendix]) : '']) }}</p>
                                <p class="mt-1 text-ink-soft">
                                    {{ __('messages.species.cites_change_note') }}
                                </p>
                            </div>
                        </section>
                    @endif
                ...
                <aside class="space-y-4">
                    <div class="relative overflow-hidden rounded-xl bg-forest-800 px-5 py-5 text-white">
                        ...
                        <h2 class="text-[1rem] font-bold">{{ __('messages.species.need_species', ['name' => $species->common_name]) }}</h2>
                        <p class="mt-1 text-[1.0625rem] text-forest-100">{{ __('messages.species.request_quote_connect') }}</p>
                        <a href="{{ route('rfq.create', ['species' => $species->slug]) }}"
                    ...
                    <div class="rounded-xl border border-sand-300/70 bg-white px-5 py-4">
                        <h2 class="text-[1.125rem] font-bold text-ink">{{ __('messages.species.at_a_glance') }}</h2>
            ...
            <section class="mt-12 pb-16" aria-labelledby="exporters-heading">
                <h2 id="exporters-heading" class="{{ $sectionTitle }}">{{ __('messages.species.verified_exporters_handling', ['name' => $species->common_name]) }}</h2>
                ...
                @else
                    <div class="mt-5 rounded-xl border border-dashed border-sand-300 bg-white p-10 text-center">
                        <p class="text-[1.0625rem] text-ink-soft">{{ __('messages.species.onboarding_exporters', ['name' => $species->common_name]) }}</p>
                        <a href="{{ route('rfq.create', ['species' => $species->slug]) }}"
                           class="mt-4 inline-flex rounded-lg bg-forest-700 px-5 py-2.5 text-[1.0625rem] font-semibold text-white transition hover:bg-forest-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-forest-500 focus-visible:ring-offset-2">
                            {{ __('messages.species.submit_an_inquiry') }}
                        </a>
                    </div>
                @endif
            </section>
