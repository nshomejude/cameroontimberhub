<x-layouts.app>
    {{-- Hero --}}
    <section class="relative overflow-hidden border-b border-sand-200 bg-gradient-to-b from-forest-50 via-sand-50 to-sand-50">
        ...
        <div class="relative mx-auto max-w-5xl px-4 py-24 text-center sm:py-28">
            ...
            <h1 class="mx-auto mt-5 max-w-4xl font-display text-5xl font-semibold leading-[1.03] text-forest-950 sm:text-6xl lg:text-7xl">
                {{ __('messages.home.hero_title') }}
            </h1>
            <p class="mx-auto mt-6 max-w-2xl text-xl leading-relaxed text-ink-soft">
                {{ __('messages.home.hero_subtitle') }}
            </p>

            {{-- Search --}}
            <form method="GET" action="{{ route('directory') }}" class="mx-auto mt-9 flex max-w-xl items-center gap-2 rounded-2xl border border-sand-200 bg-white p-2 shadow-xl shadow-forest-900/5">
                <span class="pl-3 text-ink-soft"><x-heroicon-m-magnifying-glass class="h-5 w-5" /></span>
                <input name="q" type="search" placeholder="Search verified exporters…"
                       class="min-w-0 flex-1 border-0 bg-transparent py-2.5 text-base text-ink placeholder:text-ink-soft/60 focus:outline-none focus:ring-0">
            ...
            @if($species->isNotEmpty())
                <div class="mt-5 flex flex-wrap items-center justify-center gap-2 text-sm">
                    <span class="text-ink-soft">Popular species:</span>
                    @foreach($species->take(6) as $sp)
                        <a href="{{ route('species.show', $sp->slug) }}" class="rounded-full border border-sand-200 bg-white/70 px-3 py-1 font-medium text-forest-800 transition hover:border-forest-300 hover:bg-white">{{ $sp->common_name }}</a>
                    @endforeach
                </div>
            @endif

            <dl class="mx-auto mt-16 grid max-w-2xl grid-cols-3 gap-6 border-t border-sand-200 pt-8">
                @foreach ([
                    ...
                    <div>
                        <dt class="font-display text-4xl font-semibold text-forest-800 sm:text-5xl">{{ $stat['n'] }}</dt>
                        <dd class="mt-1 text-sm text-ink-soft">{{ $stat['label'] }}</dd>
                    </div>
                @endforeach
            </dl>
        </div>
        ...
        <section class="mx-auto max-w-6xl px-4 py-20">
            <div class="flex items-end justify-between gap-4">
                <div>
                    ...
                    <h2 class="mt-3 font-display text-3xl font-semibold text-forest-950">Featured verified exporters</h2>
                </div>
                <a href="{{ route('directory') }}" class="hidden shrink-0 items-center gap-1 text-sm font-semibold text-forest-700 transition hover:text-forest-900 sm:inline-flex">
    ...
    {{-- Trust pillars --}}
    <section class="border-y border-sand-200 bg-sand-100/50">
        <div class="mx-auto max-w-6xl px-4 py-20">
            <div class="text-center">
                <p class="eyebrow">{{ __('messages.home.pillars_title') }}</p>
                <h2 class="mt-3 font-display text-3xl font-semibold text-forest-950">Trade timber with confidence</h2>
            </div>
            <div class="mt-12 grid gap-6 md:grid-cols-3">
                @foreach ([
                    ...
                    <div class="rounded-2xl border border-sand-200 bg-white p-7 shadow-[0_1px_0_rgba(0,0,0,0.02)] transition hover:border-forest-200 hover:shadow-md">
                        <span class="flex h-12 w-12 items-center justify-center rounded-xl bg-forest-50 text-forest-700">
                            <x-dynamic-component :component="'heroicon-o-' . $pillar['icon']" class="h-6 w-6" />
                        </span>
                        <h3 class="mt-5 font-display text-xl font-semibold text-forest-900">{{ __('messages.' . $pillar['title']) }}</h3>
                        <p class="mt-2 leading-relaxed text-ink-soft">{{ __('messages.' . $pillar['body']) }}</p>
                    </div>
                @endforeach
            </div>
        </div>
