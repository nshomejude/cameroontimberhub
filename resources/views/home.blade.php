...
<x-layouts.app
    ...
    <section class="relative isolate overflow-hidden bg-forest-950 lg:hidden">
        ...
        <div class="relative px-5 pb-6 pt-8">
            <p class="inline-flex rounded-full border border-forest-400/40 bg-forest-950/40 px-3 py-1 text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-forest-200">
            ...
            <p class="mt-4 max-w-[22rem] text-[0.95rem] leading-relaxed text-sand-200/90">
                {{ __('messages.home.hero_subtitle_mobile') }}
            </p>

            <div class="mt-6 space-y-3">
                <a href="{{ route('marketplace') }}"
                   class="flex w-full items-center justify-center gap-2.5 rounded-xl bg-forest-700 px-5 py-3.5 text-[0.95rem] font-semibold text-white transition hover:bg-forest-600">
                    <x-heroicon-o-shopping-bag class="h-5 w-5" /> Browse Marketplace
                </a>
                <a href="{{ route('rfq.create') }}"
                   class="flex w-full items-center justify-center gap-2.5 rounded-xl border border-white/40 bg-black/25 px-5 py-3.5 text-[0.95rem] font-semibold text-white transition hover:bg-black/40">
            ...
            <ul class="grid grid-cols-5 gap-1">
                @foreach($trustMobile as $item)
                    <li class="flex flex-col items-center gap-1.5 text-center">
                        ...
                        <span class="whitespace-pre-line text-[0.625rem] font-medium leading-tight text-sand-100">{{ $item['label'] }}</span>
                    </li>
                @endforeach
            </ul>
        </div>
    ...
    <section class="relative isolate hidden overflow-hidden bg-sand-100 lg:block">
        ...
        <div class="relative mx-auto flex max-w-[80rem] items-start justify-between gap-10 px-8 pb-14 pt-16">
            <div class="min-w-0 max-w-[48rem] flex-1">
                <p class="text-[0.8125rem] font-bold uppercase tracking-[0.11em] text-forest-700">
                    {{ __('messages.home.hero_eyebrow') }}
                </p>
                ...
                <form method="GET" action="{{ route('search') }}"
                    ...
                    <input id="hero-q" name="q" type="search" placeholder="Search timber products, species, suppliers..."
                           class="min-w-0 flex-1 border-0 bg-transparent px-4 text-[0.9375rem] text-ink placeholder:text-ink-soft/70 focus:outline-none focus:ring-0">

                    <label for="hero-species" class="sr-only">Species</label>
                    <select id="hero-species" name="species"
                            class="shrink-0 border-0 border-l border-sand-300 bg-transparent py-2.5 pl-3 pr-7 text-[0.9375rem] font-medium text-ink focus:outline-none focus:ring-0">
                    ...
                    <button type="submit"
                            class="ml-1.5 inline-flex shrink-0 items-center gap-2 rounded-lg bg-forest-700 px-7 text-[0.9375rem] font-semibold text-white transition hover:bg-forest-800">
                ...
                <div class="mt-6 flex flex-wrap items-center gap-3">
                    <a href="{{ route('marketplace') }}"
                       class="inline-flex items-center gap-2.5 rounded-lg bg-forest-700 px-7 py-3.5 text-[0.9375rem] font-semibold text-white transition hover:bg-forest-800">
                        <x-heroicon-o-shopping-bag class="h-5 w-5" /> Browse Timber Marketplace
                    </a>
                    <a href="{{ route('rfq.create') }}"
                       class="inline-flex items-center gap-2.5 rounded-lg border border-forest-300 bg-white px-7 py-3.5 text-[0.9375rem] font-semibold text-forest-800 transition hover:border-forest-600 hover:bg-forest-50">
                        <x-heroicon-o-paper-airplane class="h-5 w-5" /> Post an RFQ
                    </a>
                </div>
                ...
                <ul class="mt-9 flex flex-nowrap items-center gap-x-6">
                    @foreach($trust as $item)
                        <li class="inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap text-[0.75rem] font-medium text-ink">
                    ...
                    @foreach($stats as $key => $stat)
                        <div class="flex items-center gap-4 py-4 first:pt-0 last:pb-0">
                            ...
                            <div>
                                ...
                                <dd class="mt-1.5 text-[0.8125rem] text-sand-200/90">{{ $stat['label'] }}</dd>
                            </div>
                        </div>
                    @endforeach
                </dl>
            ...
            <form method="GET" action="{{ route('search') }}" class="mt-4 space-y-3">
                <div class="relative">
                    ...
                    <input id="m-q" name="q" type="search" placeholder="Search species, products, suppliers..."
                           class="w-full rounded-xl border border-sand-300 bg-white py-3.5 pl-12 pr-4 text-[0.9375rem] text-ink placeholder:text-ink-soft/70 focus:border-forest-600 focus:outline-none focus:ring-0">
                </div>

                <label for="m-species" class="sr-only">Species</label>
                <select id="m-species" name="species"
                        class="w-full rounded-xl border border-sand-300 bg-white px-4 py-3.5 text-[0.9375rem] text-ink focus:border-forest-600 focus:outline-none focus:ring-0">
                ...
                <button type="submit"
                        class="flex w-full items-center justify-center gap-2.5 rounded-xl bg-forest-700 px-5 py-3.5 text-[0.9375rem] font-semibold text-white transition hover:bg-forest-800">
                    <x-heroicon-o-magnifying-glass class="h-5 w-5" /> Search
                </button>
            </form>
        </div>
    ...
    <section class="mt-8 lg:hidden" aria-labelledby="categories-heading">
        <div class="flex items-center justify-between px-4">
            ...
            <a href="{{ route('marketplace') }}" class="inline-flex items-center gap-1 text-[0.875rem] font-semibold text-forest-700">
                    ...
                    <a href="{{ route('marketplace', ['type' => $category['type']->value]) }}"
                       class="block overflow-hidden rounded-2xl bg-white ring-1 ring-sand-300/70 transition hover:ring-forest-300">
                        ...
                        <div class="px-3 pb-3 pt-7">
                            <p class="text-[0.9375rem] font-bold text-ink">{{ $category['title'] }}</p>
                            <p class="mt-0.5 text-[0.8125rem] text-ink-soft">{{ $category['subtitle'] }}</p>
                        </div>
                    </a>
                </li>
            @endforeach
        ...
        <section class="hidden lg:block" aria-labelledby="products-heading">
            <div class="mx-auto max-w-[80rem] px-8 pb-4 pt-12">
                <div class="flex items-end justify-between gap-6">
                    <div>
                        <p class="text-[0.75rem] font-bold uppercase tracking-[0.11em] text-forest-700">Browse Timber Marketplace</p>
                        <h2 id="products-heading" class="mt-2 text-[1.75rem] font-bold tracking-tight text-ink">Popular Timber Products</h2>
                    </div>
                    <a href="{{ route('marketplace') }}"
                       class="shrink-0 rounded-lg border border-sand-300 bg-white px-5 py-2.5 text-[0.875rem] font-semibold text-ink transition hover:border-forest-600 hover:text-forest-800">
                        View All Products
                    </a>
                </div>
                    ...
                    @foreach($products as $product)
                        <li class="flex flex-col overflow-hidden rounded-xl bg-white ring-1 ring-sand-300/70 transition hover:ring-forest-300">
                            ...
                            <div class="flex flex-1 flex-col p-3">
                                <h3 class="text-[0.875rem] font-bold leading-snug text-ink">
                                    <a href="{{ route('products.show', $product->slug) }}" class="hover:text-forest-800">{{ $product->name }}</a>
                                </h3>
                                <p class="mt-1 text-[0.75rem] text-ink-soft">{{ $specLine($product) }}</p>
                                <p class="mt-0.5 text-[0.75rem] text-ink-soft">{{ $moqLine($product) }}</p>

                                <p class="mt-1.5 flex items-center gap-1 text-[0.75rem] text-ink">
                                    <span>Supplier:</span>
                                    <span class="font-semibold">Verified</span>
                                    <x-heroicon-s-check-circle class="h-3.5 w-3.5 text-forest-600" />
                                </p>
                                <p class="mt-1 flex items-center gap-1 text-[0.75rem] text-ink-soft">
                                ...
                                <div class="mt-auto pt-3">
                                    <a href="{{ route('rfq.create', ['product' => $product->slug]) }}"
                                       class="block rounded-md bg-forest-700 px-3 py-2 text-center text-[0.8125rem] font-semibold text-white transition hover:bg-forest-800">
                                        Request Quote
                                    </a>
                                </div>
                            </div>
    ...
    <section class="mt-8 lg:mt-0" aria-labelledby="species-heading">
        ...
        <div class="lg:hidden">
            <div class="flex items-center justify-between px-4">
                ...
                <a href="{{ route('species.index') }}" class="inline-flex items-center gap-1 text-[0.875rem] font-semibold text-forest-700">
                    View All <x-heroicon-m-chevron-right class="h-4 w-4" />
                </a>
            </div>
                ...
                @foreach($species as $sp)
                    <li class="w-[4.5rem] shrink-0 text-center">
                        <a href="{{ route('species.show', $sp->slug) }}" class="block">
                            ...
                            <span class="mt-2 block text-[0.8125rem] font-medium text-ink">{{ $sp->common_name }}</span>
                        </a>
                    </li>
                @endforeach
            </ul>
        ...
        <div class="mx-auto hidden max-w-[80rem] grid-cols-[1fr_25rem] gap-5 px-8 py-8 lg:grid">
            <div class="rounded-2xl bg-sand-100 p-6 ring-1 ring-sand-300/60">
                <div class="flex items-start justify-between gap-6">
                    ...
                    <a href="{{ route('species.index') }}"
                       class="shrink-0 rounded-lg border border-forest-300 bg-white px-4 py-2 text-[0.8125rem] font-semibold text-forest-800 transition hover:border-forest-600">
                        View All Species
                    </a>
                </div>
                    ...
                    @foreach($species->take(7) as $sp)
                        <li class="text-center">
                            <a href="{{ route('species.show', $sp->slug) }}" class="group block">
                                ...
                                <span class="mt-3 block text-[0.8125rem] font-medium text-ink">{{ $sp->common_name }}</span>
                            </a>
                        </li>
                    @endforeach
                </ul>
            ...
            <div class="relative isolate flex flex-col justify-center overflow-hidden rounded-2xl bg-forest-900 p-6">
                ...
                <div class="relative max-w-[13.5rem]">
                    ...
                    <p class="mt-2 text-[0.875rem] leading-relaxed text-sand-200/90">
                        Post an RFQ and get quotations from multiple verified suppliers.
                    </p>
                    <a href="{{ route('rfq.create') }}"
                       class="mt-5 inline-flex items-center gap-2 rounded-lg bg-forest-700 px-5 py-3 text-[0.875rem] font-semibold text-white transition hover:bg-forest-600">
        ...
        <div class="relative isolate overflow-hidden rounded-2xl bg-forest-950 p-5">
            <div class="flex items-start gap-4">
                ...
                <div class="min-w-0">
                    ...
                    <p class="mt-1.5 text-[0.8125rem] leading-relaxed text-sand-200/90">
                        Post an RFQ and get quotations from multiple verified suppliers.
                    </p>
                </div>
            </div>
            <a href="{{ route('rfq.create') }}"
               class="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-white px-5 py-3.5 text-[0.9375rem] font-semibold text-forest-800 transition hover:bg-sand-100">
            ...
            <div class="mx-auto max-w-[80rem] px-8 py-6">
                ...
                <div class="mt-5 grid grid-cols-[1fr_15rem] gap-5">
                        ...
                        @foreach($suppliers as $supplier)
                            <li class="flex flex-col rounded-xl bg-white p-4 ring-1 ring-sand-300/70">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <h3 class="truncate text-[0.9375rem] font-bold text-ink">
                                            <a href="{{ route('companies.show', $supplier->slug) }}" class="hover:text-forest-800">{{ $supplier->name }}</a>
                                        </h3>
                                        <p class="mt-0.5 truncate text-[0.75rem] text-ink-soft">{{ $supplier->city }}, Cameroon</p>
                                        <p class="mt-1.5 inline-flex items-center gap-1 text-[0.75rem] font-semibold text-forest-700">
                                ...
                                <dl class="mt-4 grid grid-cols-3 gap-1 border-t border-sand-200 pt-3 text-center">
                                    @foreach ([
                                        ...
                                        <div>
                                            <dt class="text-[0.6875rem] text-ink-soft">{{ $label }}</dt>
                                            <dd class="mt-0.5 text-[0.875rem] font-bold text-ink">{{ $value }}</dd>
                                        </div>
                                    @endforeach
                                </dl>
                                ...
                                <div class="mt-4 flex gap-2">
                                    <a href="{{ route('companies.show', $supplier->slug) }}"
                                       class="flex-1 rounded-md border border-sand-300 px-2 py-2 text-center text-[0.75rem] font-semibold text-ink transition hover:border-forest-600 hover:text-forest-800">
                                        View Company
                                    </a>
                                    <a href="{{ route('rfq.create', ['company' => $supplier->slug]) }}"
                                       class="flex-1 rounded-md bg-forest-700 px-2 py-2 text-center text-[0.75rem] font-semibold text-white transition hover:bg-forest-800">
                                        Request Quote
                                    </a>
                                </div>
                            </li>
                    ...
                    <aside class="flex flex-col rounded-xl bg-forest-800 p-5">
                        ...
                        <p class="mt-3 text-[0.8125rem] leading-relaxed text-sand-200/90">
                            Grow your business and reach international buyers on our trusted platform.
                        </p>
                        <ul class="mt-4 space-y-2">
                            @foreach($supplierPerks as $perk)
                                <li class="flex items-center gap-2 text-[0.8125rem] text-sand-100">
                                    <x-heroicon-s-check-circle class="h-4 w-4 shrink-0 text-forest-200" /> {{ $perk }}
                                </li>
                            @endforeach
                        </ul>
                        <a href="{{ route('register') }}"
                           class="mt-6 flex items-center justify-center gap-2 rounded-lg bg-white px-5 py-3 text-[0.875rem] font-semibold text-ink transition hover:bg-sand-100">
    ...
    <section class="mt-8 lg:mt-2" aria-labelledby="how-heading">
        <div class="mx-auto max-w-[80rem] px-4 lg:px-8">
            <div class="rounded-2xl bg-sand-100 px-4 py-7 lg:px-8">
                <h2 id="how-heading" class="text-center text-[1.1875rem] font-bold tracking-tight text-ink lg:text-[0.8125rem] lg:uppercase lg:tracking-[0.11em] lg:text-forest-800">
                ...
                <ol class="mt-6 grid grid-cols-3 gap-x-2 gap-y-7 lg:hidden">
                    @foreach($steps as $i => $step)
                        <li class="relative text-center">
                            ...
                            <p class="mt-2.5 text-[0.8125rem] font-bold text-ink">{{ $step['title'] }}</p>
                            <p class="mt-1 text-[0.6875rem] leading-snug text-ink-soft">{{ $step['body'] }}</p>
                        </li>
                    @endforeach
                </ol>
                ...
                <ol class="mt-5 hidden items-start lg:flex">
                    @foreach($steps as $i => $step)
                        <li class="flex-1 px-1 text-center">
                            ...
                            <p class="mt-3 text-[0.875rem] font-bold text-ink">{{ $step['title'] }}</p>
                            <p class="mx-auto mt-1 max-w-[9rem] text-[0.75rem] leading-snug text-ink-soft">{{ $step['body'] }}</p>
                        </li>
                        @if($i < count($steps) - 1)
            ...
            <ul class="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-6 lg:gap-3">
                @foreach($reasons as $reason)
                    <li class="flex items-start gap-3 rounded-xl bg-white p-4 ring-1 ring-sand-300/70">
                        ...
                        <div class="min-w-0">
                            <h3 class="text-[0.875rem] font-bold leading-snug text-ink lg:text-[0.8125rem]">{{ $reason['title'] }}</h3>
                            <p class="mt-1 text-[0.75rem] leading-snug text-ink-soft lg:text-[0.6875rem]">{{ $reason['body'] }}</p>
                        </div>
                    </li>
                @endforeach
            </ul>
        ...
        <dl class="grid grid-cols-4 gap-1 rounded-2xl bg-forest-950 px-3 py-4">
            @foreach($stats as $key => $stat)
                <div class="flex flex-col items-center gap-1.5 text-center">
                    ...
                    <dd class="text-[0.625rem] leading-tight text-sand-200/90">{{ $stat['label'] }}</dd>
                </div>
            @endforeach
        </dl>
    </section>
    ...
    <section class="mt-10 lg:mt-12" aria-labelledby="faq-heading">
        <div class="mx-auto max-w-[80rem] px-4 lg:px-8">
            ...
            <p class="mx-auto mt-2 max-w-[42rem] text-center text-[0.875rem] leading-relaxed text-ink-soft">
                ...
                @foreach($faqs as $faq)
                    <details class="group rounded-xl bg-white p-5 ring-1 ring-sand-300/70 open:ring-forest-300">
                        <summary class="flex cursor-pointer list-none items-start justify-between gap-4">
                            <h3 class="text-[0.9375rem] font-bold leading-snug text-ink">{{ $faq['q'] }}</h3>
                            <x-heroicon-o-chevron-down class="mt-0.5 h-5 w-5 shrink-0 text-forest-700 transition group-open:rotate-180" />
                        </summary>
                        <p class="mt-3 text-[0.875rem] leading-relaxed text-ink-soft">{{ $faq['a'] }}</p>
                    </details>
                @endforeach
            </div>
        </div>
    ...
    <section class="mt-9 px-4 pb-10 lg:hidden">
        <div class="relative isolate overflow-hidden rounded-2xl bg-forest-950 p-5">
            ...
            <div class="relative">
                ...
                <p class="mt-2 text-[0.8125rem] leading-relaxed text-sand-200/90">
                    Join thousands of businesses worldwide who trust Cameroon Timber Hub.
                </p>
                <div class="mt-5 space-y-3">
                    <a href="{{ route('marketplace') }}"
                       class="flex w-full items-center justify-center gap-2.5 rounded-xl bg-forest-700 px-5 py-3.5 text-[0.9375rem] font-semibold text-white transition hover:bg-forest-600">
                        <x-heroicon-o-shopping-bag class="h-5 w-5" /> Browse Marketplace
                    </a>
                    <a href="{{ route('register') }}"
                       class="flex w-full items-center justify-center gap-2.5 rounded-xl border border-white/40 px-5 py-3.5 text-[0.9375rem] font-semibold text-white transition hover:bg-white/10">
    ...
    <section class="relative isolate mt-10 hidden overflow-hidden bg-forest-900 lg:block">
        ...
        <div class="relative mx-auto flex max-w-[80rem] items-center gap-10 px-8 py-10">
            <div class="min-w-0 flex-1">
                ...
                <p class="mt-3 max-w-[28rem] text-[0.875rem] leading-relaxed text-sand-200/90">
            ...
            <div class="flex shrink-0 items-center gap-4">
                <a href="{{ route('marketplace') }}"
                   class="inline-flex items-center gap-2.5 rounded-lg bg-white px-8 py-3.5 text-[0.9375rem] font-semibold text-forest-800 transition hover:bg-sand-100">
                    <x-heroicon-o-shopping-bag class="h-5 w-5" /> Browse Marketplace
                </a>
                <a href="{{ route('rfq.create') }}"
                   class="inline-flex items-center gap-2.5 rounded-lg border border-white/60 px-8 py-3.5 text-[0.9375rem] font-semibold text-white transition hover:bg-white/10">
                    <x-heroicon-o-paper-airplane class="h-5 w-5" /> Post an RFQ
                </a>
            </div>
        </div>
