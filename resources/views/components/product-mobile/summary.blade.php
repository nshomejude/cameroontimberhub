...
<section class="px-4 pt-5">
    ...
    @if (filled($product->description))
        <p class="mt-3 text-[1.125rem] leading-relaxed text-ink-soft">{{ \Illuminate\Support\Str::limit(strip_tags((string) $product->description), 260) }}</p>
    @endif
        ...
        <ul class="mt-4 grid grid-cols-2 gap-3" role="list">
            @foreach ($chips as $chip)
                <li class="flex items-center gap-2.5 rounded-xl border border-sand-200 bg-sand-100 px-3 py-2.5">
                    ...
                    <div class="min-w-0">
                        <p class="text-[0.9375rem] leading-tight text-ink-soft">{{ $chip['label'] }}</p>
                        <p class="truncate text-[1.125rem] font-semibold leading-snug text-ink">{{ $chip['value'] }}</p>
                    </div>
                </li>
            @endforeach
        </ul>
