...
<article class="relative flex flex-col gap-5 rounded-xl border border-sand-300/70 bg-white p-4 transition hover:border-forest-200 hover:shadow-md sm:flex-row">
    ...
    <div class="relative w-full shrink-0 sm:w-56">
        ...
        @if ($species->isPremium())
            <span class="absolute bottom-2 left-2 rounded-md bg-forest-800 px-2 py-1 text-[0.6875rem] font-semibold text-white">Premium</span>
        @elseif ($species->is_promoted)
            <span class="absolute bottom-2 left-2 rounded-md bg-white/95 px-2 py-1 text-[0.6875rem] font-semibold text-forest-800">Promoted</span>
        @endif
    </div>
    ...
    <div class="min-w-0 flex-1">
            ...
            <div class="min-w-0 flex-1">
                <h3 class="flex flex-wrap items-center gap-x-2 gap-y-1 text-[1.0625rem] font-bold leading-tight text-ink">
                    ...
                    @if ($species->commercial_category)
                        <span class="inline-flex items-center rounded-full bg-forest-50 px-2 py-0.5 text-[0.625rem] font-semibold text-forest-800">{{ $species->commercial_category->shortLabel() }}</span>
                    @endif
                    @if ($species->is_cites_listed)
                        <span class="inline-flex items-center rounded-full bg-timber-100 px-2 py-0.5 text-[0.625rem] font-semibold text-timber-800">CITES{{ $species->cites_appendix ? ' '.$species->cites_appendix : '' }}</span>
                    @endif
                </h3>
                @if ($species->scientific_name)
                    <p class="mt-1 text-[0.8125rem] italic text-ink-soft">{{ $species->scientific_name }}</p>
                @endif
            </div>
        ...
        @if ($species->description)
            <p class="mt-2 line-clamp-2 text-[0.8125rem] leading-relaxed text-ink-soft">{{ strip_tags($species->description) }}</p>
        @endif
            ...
            <dl class="mt-3 flex flex-wrap items-baseline gap-x-6 gap-y-1">
                @foreach ($facts as $fact)
                    <div class="flex items-baseline gap-1.5">
                        <dt class="text-[0.6875rem] uppercase tracking-wide text-ink-soft">{{ $fact['label'] }}</dt>
                        <dd class="text-[0.8125rem] font-semibold text-ink">{{ $fact['value'] }}</dd>
                    </div>
                @endforeach
            </dl>
        @endif
        ...
        @if ($uses->isNotEmpty())
            <ul class="mt-3 flex flex-wrap gap-1.5">
                @foreach ($uses as $use)
                    <li class="rounded-full bg-sand-100 px-2.5 py-1 text-[0.6875rem] font-medium text-ink-soft">{{ $use }}</li>
                @endforeach
            </ul>
        @endif
        ...
        <div class="mt-4 flex flex-wrap items-center gap-3">
            ...
            <a href="{{ $url }}"
               class="relative z-10 ml-auto rounded-lg bg-forest-700 px-4 py-2 text-[0.8125rem] font-semibold text-white transition hover:bg-forest-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-forest-500 focus-visible:ring-offset-2">
                View Details
            </a>
        </div>
    </div>
