...
<article class="relative flex gap-5 rounded-xl border border-sand-300/70 bg-white p-4 transition hover:border-forest-200 hover:shadow-md">
    ...
    <div class="relative hidden w-56 shrink-0 sm:block">
        ...
        @if ($company->is_featured)
            <span class="absolute left-0 top-2 rounded-r-md bg-forest-700 px-2 py-0.5 text-[0.625rem] font-semibold text-white">Featured</span>
        @endif
    </div>
    ...
    <div class="min-w-0 flex-1">
            ...
            <div class="min-w-0 flex-1">
                <h3 class="flex flex-wrap items-center gap-x-2 gap-y-1 text-[1.0625rem] font-bold leading-tight text-ink">
                    ...
                    @if ($company->supplier_type)
                        <span class="inline-flex items-center rounded-full bg-sand-200 px-2 py-0.5 text-[0.625rem] font-semibold text-ink-soft">{{ $company->supplier_type->label() }}</span>
                    @endif
                </h3>

                @if ($location !== '')
                    <p class="mt-1 flex items-center gap-1 text-[0.8125rem] text-ink-soft">
        ...
        @if ($company->description)
            <p class="mt-2 line-clamp-2 text-[0.8125rem] leading-relaxed text-ink-soft">{{ $company->description }}</p>
        @endif

        @if ($specialisations->isNotEmpty())
            <ul class="mt-3 flex flex-wrap gap-1.5">
                @foreach ($specialisations as $item)
                    <li class="rounded-full bg-sand-100 px-2.5 py-1 text-[0.6875rem] font-medium text-ink-soft">{{ $item }}</li>
                @endforeach
            </ul>
        @endif
                ...
                <dl class="flex flex-wrap items-center gap-x-6 gap-y-1">
                    @foreach ($stats as $stat)
                        <div class="flex items-baseline gap-1.5">
                            <dd class="text-[0.9375rem] font-bold text-ink">{{ $stat['value'] }}</dd>
