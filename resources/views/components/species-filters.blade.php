...
<div class="flex h-full flex-col">
    <div class="flex-1 space-y-1 overflow-y-auto px-5 py-5">
        ...
        <div class="flex items-center justify-between pb-2">
            <h2 class="text-[1.125rem] font-bold text-ink">{{ __('messages.species.filter_species_title') }}</h2>
        ...
        {{-- Search species --}}
        <div class="pb-4">
            <label for="{{ $fid('search') }}" class="text-[1.0625rem] font-bold text-ink">{{ __('messages.species.search_species_label') }}</label>
            <div class="relative mt-2.5">
                <input id="{{ $fid('search') }}" type="search" wire:model.live.debounce.400ms="search"
                       placeholder="{{ __('messages.species.search_by_name_or_scientific') }}" class="{{ $facetSearch }}">
        ...
        <div class="divide-y divide-sand-200 border-t border-sand-200">
            ...
            {{-- Category --}}
            @if ($categoryFacets)
                <x-directory-accordion :title="__('messages.species.category')" :id="$fid('acc-category')" open>
                    <fieldset>
                        <legend class="sr-only">{{ __('messages.species.commercial_category') }}</legend>
                        <label class="{{ $row }}" for="{{ $fid('cat-all') }}">
                            ...
                            <span class="flex-1">{{ __('messages.species.all_categories') }}</span>
            ...
            {{-- Properties --}}
            @if ($propertyFacets)
                <x-directory-accordion :title="__('messages.species.properties')" :id="$fid('acc-properties')" open>
                    <fieldset>
                        <legend class="sr-only">{{ __('messages.species.properties') }}</legend>
            ...
            @if ($applicationFacets)
                <x-directory-accordion :title="__('messages.species.applications')" :id="$fid('acc-applications')" open>
                    <fieldset x-data="{ expanded: false }">
                        <legend class="sr-only">{{ __('messages.species.applications') }}</legend>
                        ...
                        @if (count($applicationFacets) > $facetLimit)
                            <button type="button" @click="expanded = !expanded" :aria-expanded="expanded ? 'true' : 'false'"
                                ...
                                <span x-text="expanded ? @js(__('messages.common.view_less')) : @js(__('messages.common.view_more'))">{{ __('messages.common.view_more') }}</span>
            ...
            {{-- Availability --}}
            @if ($inStockFacet)
                <x-directory-accordion :title="__('messages.species.availability')" :id="$fid('acc-availability')" open>
            ...
            @if ($regions->isNotEmpty())
                <x-directory-accordion :title="__('messages.species.origin_region')" :id="$fid('acc-region')" open>
                    <div class="relative pb-1">
                        <label for="{{ $fid('region') }}" class="sr-only">{{ __('messages.species.origin_region') }}</label>
                        <select id="{{ $fid('region') }}" wire:model.live="region"
                            ...
                            <option value="">{{ __('messages.species.select_region') }}</option>
