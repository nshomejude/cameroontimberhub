    ...
    @if ($title)
        <div class="flex items-center gap-3 px-5 pt-5 {{ $padded ? '' : 'pb-4' }}">
            <div class="min-w-0">
                ...
                @if ($subtitle)
                    <p class="mt-0.5 text-[0.8125rem] text-ink-soft">{{ $subtitle }}</p>
                @endif
            </div>
            @if ($href)
                <a href="{{ $href }}" class="ml-auto shrink-0 text-[0.8125rem] font-semibold text-forest-700 transition hover:text-forest-900">{{ $linkLabel }}</a>
            @endif
        </div>
    @endif
